...
const addToWorkArray = (instance, minionId) => {
  const work = findDataArrayByName('work');
  if (work === null) {
    return null;
  }
  if(work.isValid(instance)){
    instance.minionId = minionId;
    instance.id = `${work.nextId++}`;
    work.data.push(instance);
    return work.data[work.data.length - 1];
  }
}
...
const updateWorkById = (instance, workId, minionId) => {
  const work = findDataArrayByName('work');
  if (work === null) {
    return null;
  }
  const instanceIndex = work.data.findIndex((element) => {
    return element.id === workId;
  })
  const validMinionId = work.data.find((element) => {
    return element.minionId === minionId
  });

  if(instanceIndex > -1 && validMinionId && !isNaN(workId) && work){
    work.data[instanceIndex] = instance;
    return work.data[instanceIndex];
  }else{
    return null;
  }
}
...
const deleteFromWorkArrayById = (workId) => {
  const work = findDataArrayByName('work');
  if (work === null) {
    return null;
  }
  let j = 0
  const index = work.data.findIndex((element) => {
    return element.id === workId
  })
  while(j < work.data.length){
    if(work.data[j].id === workId){
      work.data.splice(j, 1);
      j -= 1
    }
    j += 1;
  }
...
module.exports = {
  ...
  isValidWork,
  getAllFromDatabase,
  getWorkFromMinionsDb,
  updateWorkById,
  getFromDatabaseById,
  addToDatabase,
  addToWorkArray,
  deleteFromWorkArrayById,
